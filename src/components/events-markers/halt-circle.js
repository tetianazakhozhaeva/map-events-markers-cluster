...
class HaltCircle extends React.Component {
  ...
  render() {
    ...
    const processClickHandler =
      DEVICE_TYPE === PHONE ? this.mouseOver : this.click;
  ...
  showOrHidePopupHaltMarker = isPopupVisible => {
    ...
    if (isPopupVisible) {
      const isBtnReturnInvisible =
        Object.keys(this.state.map).length > 0
          ? this.state.isReturnButtonInvisible
          : true;
  ...
  moveEnd = () => {
    ...
    if (!this._isAfterClick) this._isPopupVisible = false;

    if (DEVICE_TYPE !== PHONE) this.props.hidePopupInfo();
  };
}
